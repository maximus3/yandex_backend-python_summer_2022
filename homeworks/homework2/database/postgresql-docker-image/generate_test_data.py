...
from mimesis.types import JSON, CallableSchema
...
def get_user_schema(
    provider: Person, *, doctor: bool = False
) -> CallableSchema:
...
if __name__ == '__main__':
    ...
    with open(get_filepath(MEDICINES_FILENAME)) as csvfile:
        ...
        for row in reader:
            ...
            item = {
                'name': ''.join(
                    char for char in row['tradename'] if char not in '(*)'
                ),
    ...
    doctor_schema.to_csv(create_filepath('doctors.csv'), iterations=40)  # type: ignore
